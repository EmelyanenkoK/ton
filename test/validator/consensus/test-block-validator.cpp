 ...
 protected:
  ...
  void run() final {
    ts_.run([this]() -> td::actor::Task<td::Unit> {
      ...
      genesis_ = make_normal_state(ctx().shard(), 1, min_mc_block_id);
      handle_.publish<Start>(genesis_);
...
struct AcceptsGoodEmptyCandidates : BlockValidatorTest {
  td::actor::Task<td::Unit> run_test() override {
    auto block_id = genesis_->as_normal().value();
    auto candidate = make_empty_candidate(block_id, PeerValidatorId{0});

    auto result = co_await handle_.publish<ValidationRequest>(genesis_, candidate);
...
struct AcceptsEmptyCandidateWhenResolvedStateMatchesReference : BlockValidatorTest {
  td::actor::Task<td::Unit> run_test() override {
    ...
    // ValidationRequest already carries the genesis_ resolved from candidate->parent_id upstream, so
    // this actor only checks that the empty candidate references that supplied genesis_ block and does
    ...
    auto candidate = td::make_ref<Candidate>(CandidateId{.slot = 1, .hash = bits256_pattern(1701)},
                                             ...
                                             genesis_->as_normal().value(), td::BufferSlice());
...
struct RejectsEmptyCandidateWithWrongBlockIdExtReference : BlockValidatorTest {
  td::actor::Task<td::Unit> run_test() override {
    ...
    auto expected = genesis_->as_normal().value();
    auto wrong_block =
...
struct WaitsUntilGenUtime : BlockValidatorTest {
  td::actor::Task<td::Unit> run_test() override {
    ...
    auto task = handle_.publish<ValidationRequest>(genesis_, candidate).start();

    co_await ts_.wait_sync_work();

    EXPECT(!task.await_ready());
    EXPECT_APPROX(ts_.next_timeout_in(), 5.0);
...
struct RejectsStaleMasterchainCandidates : MasterchainBlockValidatorTest {
  td::actor::Task<td::Unit> run_test() override {
    ...
    auto result = co_await handle_.publish<ValidationRequest>(genesis_, candidate).wrap();
