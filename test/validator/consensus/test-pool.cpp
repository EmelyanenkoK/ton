...
struct StandstillTriggersAtConfiguredTimeout : PoolTest {
  ...
  void configure_bus(PoolBus& bus) override {
    ...
    bus.config.noncritical_params.standstill_timeout = std::chrono::milliseconds{5000};
...
struct WaitsForPersistentStoreBeforeUsingLocalVote : PoolTest {
  td::actor::Task<td::Unit> run_test() override {
    ...
    for (size_t i = 0; i < 3; ++i) {
      ...
      auto serialized = simplex::Signed<simplex::Vote>{signed_vote.validator, simplex::Vote{signed_vote.vote},
...
struct DeterministicInterleavingStressPreservesPoolState : PoolTest {
  td::actor::Task<td::Unit> run_test() override {
    ...
    for (size_t i = 0; i < 3; ++i) {
      ...
      messages.emplace_back(PeerValidatorId{i}, ProtocolMessage{simplex::Signed<simplex::Vote>{
                                                    signed_notar.validator, simplex::Vote{signed_notar.vote},
                                                    signed_notar.signature.clone()}
                                                                    .serialize()});
      auto signed_skip = make_signed_simplex_vote(ctx(), *bus_, i, simplex::SkipVote{1});
      messages.emplace_back(PeerValidatorId{i},
                            ProtocolMessage{simplex::Signed<simplex::Vote>{
                                signed_skip.validator, simplex::Vote{signed_skip.vote}, signed_skip.signature.clone()}
                                                .serialize()});
    }
    ...
    messages.emplace_back(PeerValidatorId{0},
                          ProtocolMessage{simplex::Signed<simplex::Vote>{
                              dup_notar.validator, simplex::Vote{dup_notar.vote}, dup_notar.signature.clone()}
                                              .serialize()});
    auto dup_skip = make_signed_simplex_vote(ctx(), *bus_, 1, simplex::SkipVote{1});
    messages.emplace_back(PeerValidatorId{1},
                          ProtocolMessage{simplex::Signed<simplex::Vote>{
                              dup_skip.validator, simplex::Vote{dup_skip.vote}, dup_skip.signature.clone()}
                                              .serialize()});
...
struct NoncriticalParamsUpdateChangesStandstillTimeout : PoolTest {
  td::BufferSlice final_payload_;

  void configure_bus(PoolBus& bus) override {
    bus.config.noncritical_params.standstill_timeout = std::chrono::milliseconds{5000};
    auto final_cert =
        make_simplex_certificate(ctx(), bus, simplex::FinalizeVote{make_candidate_id(0, 8000)}, {0, 1, 2});
    final_payload_ = final_cert->serialize();
    bus.bootstrap_certificates.push_back(std::move(final_cert.unique_write()).consume_and_upcast());
  }

  td::actor::Task<td::Unit> run_test() override {
    // Standstill resolution reschedules using params_.standstill_timeout. Verify that after
    // a NoncriticalParamsUpdated, the *rescheduled* alarm (after the first one fires) uses
    // the new timeout.
    auto state = make_normal_state(ctx().shard(), 1, min_mc_block_id);
    ...
    // Shorten standstill_timeout to 1s for the next reschedule.
    auto new_params = bus_->config.noncritical_params;
    new_params.standstill_timeout = std::chrono::milliseconds{1000};
    handle_.publish<NoncriticalParamsUpdated>(new_params);
    co_await ts_.wait_sync_work();

    // First alarm still fires at the original ~5s.
    auto initial_timeout = ts_.next_timeout_in();
    EXPECT(initial_timeout > 4.0);
    ...
    // The next alarm should be ~1s, not ~5s.
    auto rescheduled_timeout = ts_.next_timeout_in();
    EXPECT(rescheduled_timeout <= 1.5);
