...
struct TestMessageFilters {
  std::optional<size_t> drop_final_vote_dst_node;
  std::optional<td::uint32> drop_final_vote_from_slot;
...
std::string format_windows(const std::set<td::uint32>& windows) {
  td::StringBuilder sb;
  sb << "[";
  bool first = true;
...
td::Status verify_skip_timeout_uses_last_finalization(const TraceSnapshot& snapshot) {
  ...
  if (!later_finalization_windows.empty()) {
    return td::Status::Error(PSTRING() << "Rule 6 scenario failed to freeze validator #0.0 finalization after window "
                                       ...
                                       << format_windows(later_finalization_windows));
  }
  ...
  if (!saw_stalled_window) {
    return td::Status::Error(PSTRING() << "Rule 6 scenario never reached stalled window "
                                       ...
                                       << format_windows(observed_leader_windows));
  }
  ...
  double spec_lower_bound_s = FIRST_BLOCK_TIMEOUT_S * std::pow(FIRST_BLOCK_TIMEOUT_MULTIPLIER,
                                                               *expected_stalled_window - *last_finalized_window - 1);
...
std::optional<ProtocolVoteTraceRecord> try_decode_protocol_vote(const TestSimplexBus& bus, size_t src_node_idx,
  ...
  auto maybe_vote = simplex::Signed<simplex::Vote>::from_tl(std::move(*maybe_tl_vote.move_as_ok()),
                                                            PeerValidatorId{src_node_idx}, bus);
...
bool should_drop_protocol_message(const TestSimplexBus& bus, size_t dst_node_idx, td::Slice data) {
  auto maybe_vote = try_decode_protocol_vote(bus, bus.local_id.idx.value(), 0, dst_node_idx, data);
  if (maybe_vote.has_value()) {
    std::scoped_lock lock(test_message_filters_mutex);
    if (auto* final = std::get_if<simplex::FinalizeVote>(&maybe_vote->vote.vote)) {
      return test_message_filters.drop_final_vote_dst_node == dst_node_idx &&
             test_message_filters.drop_final_vote_from_slot.has_value() &&
             final->id.slot >= *test_message_filters.drop_final_vote_from_slot;
    }
    return false;
  }
...
class TestConsensus : public td::actor::Actor {
 public:
  ...
  td::actor::Task<> set_drop_final_votes_for_test(size_t dst_node_idx, td::uint32 from_slot) {
    std::scoped_lock lock(test_message_filters_mutex);
    test_message_filters.drop_final_vote_dst_node = dst_node_idx;
    test_message_filters.drop_final_vote_from_slot = from_slot;
    co_return td::Unit{};
  }
 ...
 private:
  ...
  void start_instance(size_t node_idx, size_t instance_idx) {
    ...
    bus->config =
        NewConsensusConfig{.target_rate_ms = TARGET_RATE_MS,
                           .max_block_size = 1 << 20,
