...
struct Rule6SkipTimeoutUsesLastObservedFinalization : SimplexConsensusTest {
  ...
  td::actor::Task<td::Unit> run_test() override {
    ...
    double expected_window2_min_s =
        options().target_rate_ms / 1000. +
        options().first_block_timeout_ms / 1000. * handle_->config.noncritical_params.first_block_timeout_multiplier;
...
struct NoncriticalParamsUpdateChangesTargetRate : SimplexConsensusTest {
  TestOptions options() const override {
    auto opts = SimplexConsensusTest::options();
    opts.weight_distribution = {1, 1};
    opts.slots_per_leader_window = 1;
  ...
  td::actor::Task<td::Unit> run_test() override {
    // After NoncriticalParamsUpdated with a new target_rate, the next window's
    // skip timeout should reflect the updated rate.
    auto base = ParentId{make_candidate_id(0, 5000)};
    auto parent_state = make_normal_state(ctx().shard(), 20, min_mc_block_id);
    observer().resolve_state_results_.push_back(simplex::ResolveState::Result{.state = parent_state});

    // Window 0: observe skip timeout with original target_rate=1000ms.
    // alarm = first_block_timeout(1s) + target_rate(1s) = 2s from now.
    co_await handle_.publish<simplex::LeaderWindowObserved>(0, base);
    co_await ts_.wait_sync_work();
    double timeout_before = ts_.next_timeout_in();

    // Skip window 0.
    ts_.advance_time(timeout_before);
    co_await ts_.wait_sync_work();

    // Update target_rate from 1000ms to 200ms.
    auto new_params = bus_->config.noncritical_params;
    new_params.target_rate = std::chrono::milliseconds{200};
    handle_.publish<NoncriticalParamsUpdated>(new_params);
    co_await ts_.wait_sync_work();

    // Window 1: alarm = first_block_timeout(1.2s, grown due to skip) + target_rate(0.2s) = 1.4s.
    observer().resolve_state_results_.push_back(simplex::ResolveState::Result{.state = parent_state});
    co_await handle_.publish<simplex::LeaderWindowObserved>(1, base);
    co_await ts_.wait_sync_work();
    double timeout_after = ts_.next_timeout_in();

    // timeout_before was ~2.0s, timeout_after should be ~1.4s.
    EXPECT(timeout_after < timeout_before);

    co_return {};
  }
};
REGISTER_TEST(SimplexConsensus, NoncriticalParamsUpdateChangesTargetRate);

struct NoncriticalParamsUpdateChangesFirstBlockTimeout : SimplexConsensusTest {
  TestOptions options() const override {
    auto opts = SimplexConsensusTest::options();
    opts.weight_distribution = {1, 1};
    opts.slots_per_leader_window = 1;
  ...
  td::actor::Task<td::Unit> run_test() override {
    // After NoncriticalParamsUpdated with a new first_block_timeout, the next
    // no-skip window should reset first_block_timeout_ to the new value.
    auto base = ParentId{make_candidate_id(0, 5100)};
    auto parent_state = make_normal_state(ctx().shard(), 20, min_mc_block_id);

    // Window 0: alarm = first_block_timeout(1s) + target_rate(1s) = 2s.
    observer().resolve_state_results_.push_back(simplex::ResolveState::Result{.state = parent_state});
    co_await handle_.publish<simplex::LeaderWindowObserved>(0, base);
    co_await ts_.wait_sync_work();
    double timeout_before = ts_.next_timeout_in();

    // Update first_block_timeout from 1000ms to 3000ms.
    auto new_params = bus_->config.noncritical_params;
    new_params.first_block_timeout = std::chrono::milliseconds{3000};
    handle_.publish<NoncriticalParamsUpdated>(new_params);
    co_await ts_.wait_sync_work();

    // Window 1 (no skip in between): first_block_timeout_ resets to params_.first_block_timeout = 3s.
    // alarm = first_block_timeout(3s) + target_rate(1s) = 4s.
    observer().resolve_state_results_.push_back(simplex::ResolveState::Result{.state = parent_state});
    co_await handle_.publish<simplex::LeaderWindowObserved>(1, base);
    co_await ts_.wait_sync_work();
    double timeout_after = ts_.next_timeout_in();

    // timeout_before was ~2.0s, timeout_after should be ~4.0s.
    EXPECT(timeout_after > timeout_before);
    EXPECT(timeout_after >= 3.5);

    co_return {};
  }
};
REGISTER_TEST(SimplexConsensus, NoncriticalParamsUpdateChangesFirstBlockTimeout);

struct NoncriticalParamsUpdateChangesMaxLeaderWindowDesync : SimplexConsensusTest {
  TestOptions options() const override {
    auto opts = SimplexConsensusTest::options();
    opts.weight_distribution = {1, 1};
    opts.slots_per_leader_window = 1;
  ...
  td::actor::Task<td::Unit> run_test() override {
    // Candidate from a far-future window should be accepted up to max_leader_window_desync,
    // then dropped after reducing the limit via NoncriticalParamsUpdated.
    auto base = ParentId{make_candidate_id(0, 5200)};
    auto parent_state = make_normal_state(ctx().shard(), 25, min_mc_block_id);
    // resolve_state_results: 1 for generation, 1 for candidate_ok's try_notarize.
    observer().resolve_state_results_.push_back(simplex::ResolveState::Result{.state = parent_state});
    observer().resolve_state_results_.push_back(simplex::ResolveState::Result{.state = parent_state});
    observer().validation_results_.push_back(CandidateAccept{});
    ...
    // Candidate in window 2 (slot 2) should be accepted with max_leader_window_desync=2.
    auto candidate_ok = make_wait_candidate(make_candidate_id(2, 5202), base, PeerValidatorId{1});
    handle_.publish<CandidateReceived>(candidate_ok);
    co_await ts_.wait_sync_work();
    EXPECT_EQ(observer().wait_for_parent_candidate_ids_.size(), static_cast<size_t>(1));

    // Now reduce max_leader_window_desync to 0.
    auto new_params = bus_->config.noncritical_params;
    new_params.max_leader_window_desync = 0;
    handle_.publish<NoncriticalParamsUpdated>(new_params);
    co_await ts_.wait_sync_work();

    // Candidate in window 1 (slot 1) should now be dropped as too new.
    auto candidate_dropped = make_wait_candidate(make_candidate_id(1, 5201), base, PeerValidatorId{1});
    handle_.publish<CandidateReceived>(candidate_dropped);
    co_await ts_.wait_sync_work();

    // Still only 1 candidate was processed — the second was dropped.
    EXPECT_EQ(observer().wait_for_parent_candidate_ids_.size(), static_cast<size_t>(1));

    co_return {};
  }
};
REGISTER_TEST(SimplexConsensus, NoncriticalParamsUpdateChangesMaxLeaderWindowDesync);
