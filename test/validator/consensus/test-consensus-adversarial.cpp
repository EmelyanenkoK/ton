...
td::Result<TestCase> parse_test_case(td::Slice s) {
  ...
  if (s == "offline-cohort-network-handoff-still-finalizes") {
    return TestCase::OfflineCohortNetworkHandoffStillFinalizes;
  }
...
std::chrono::milliseconds STANDSTILL_TIMEOUT{10'000};
std::chrono::milliseconds FIRST_BLOCK_TIMEOUT{1'000};
...
std::vector<size_t> cohort_handoff_second_offline_nodes() {
  return {4, 5, 6};
}
...
td::Status verify_adversarial_invariants(const TraceSnapshot& snapshot) {
  ...
  for (const auto& record : snapshot.protocol_certificates) {
    ...
    if (auto* final = std::get_if<simplex::FinalizeVote>(&record.vote.vote)) {
      ...
      if (!inserted && it->second != final->id) {
        return td::Status::Error(PSTRING() << "conflicting finalization certificates for slot " << final->id.slot
                                           << ": first=" << it->second << ", later=" << final->id);
      }
    }
  }
...
td::Status verify_candidate_resolution_recovery(const TraceSnapshot& snapshot) {
  ...
  if (requests.front().timeout_s < 0.35 || requests.front().timeout_s > 0.65) {
    return td::Status::Error(PSTRING() << "unexpected initial requestCandidate timeout for target " << *expected_target
                                       << ": " << requests.front().timeout_s << "s");
  }
  ...
  for (size_t i = 1; i < requests.size(); ++i) {
    ...
    if (requests[i].timeout_s + 0.05 < min_expected_timeout) {
      return td::Status::Error(PSTRING() << "requestCandidate timeout for target " << *expected_target
                                         ...
                                         << min_expected_timeout << "s, got " << requests[i].timeout_s << "s");
    }
  ...
  if (!resolved.has_value()) {
    return td::Status::Error(PSTRING() << "recovery scenario showed backoff for candidate " << *expected_target
  ...
  for (const auto& record : snapshot.protocol_votes) {
    ...
    if (notar_it->second > record.ts + 1e-9) {
      return td::Status::Error(
          PSTRING() << "validator #" << record.src_node_idx << "." << record.src_instance_idx
                    << " voted Notar for child " << notar->id << " before observing parent notarization for "
                    << parent_id << ": parent_notar_ts=" << notar_it->second << ", child_notar_ts=" << record.ts);
    }
  ...
  for (const auto& [empty_id, finalization_ts] : finalized_empty_ts) {
    ...
    if (accepted_it == first_accepted_ts_by_block.end()) {
      return td::Status::Error(PSTRING() << "referenced block " << block_it->second.to_str()
                                         << " for finalized empty candidate " << empty_id << " was never accepted");
    }
    if (accepted_it->second > finalization_ts + 1e-9) {
      return td::Status::Error(PSTRING() << "empty candidate " << empty_id << " finalized before its referenced block "
                                         << block_it->second.to_str() << " was accepted");
    }
    for (const auto& record : snapshot.finalizations_observed) {
...
td::Status verify_standstill_rebroadcast_contents(const TraceSnapshot& snapshot) {
  ...
  double first_standstill_attempt_ts =
      last_finalization_ts + static_cast<double>(STANDSTILL_TIMEOUT.count()) / 1000.0 * 0.75;
    ...
    for (td::uint32 slot : skip_slots) {
      if (final_it->second.contains(slot)) {
        return td::Status::Error(PSTRING()
                                 << "validator #" << validator << " sent both Skip and Final for slot " << slot);
      }
    }
  }
...
td::Status verify_certificate_requires_quorum(const TraceSnapshot& snapshot) {
  ...
  for (const auto& record : snapshot.protocol_certificates) {
    ...
    if (signer_weight < quorum_weight) {
      return td::Status::Error(PSTRING() << "certificate for " << record.vote << " had insufficient signer weight: got="
                                         << signer_weight << ", quorum=" << quorum_weight);
    }
  }
  return td::Status::OK();
}
...
td::Status verify_skip_timeout_uses_last_finalization(const TraceSnapshot& snapshot) {
  ...
  if (!saw_stalled_window) {
    return td::Status::Error(PSTRING() << "Rule 6 scenario never reached stalled window " << *expected_stalled_window
                                       << " on validator #0.0; observed leader windows="
  ...
  for (const auto& record : snapshot.protocol_votes) {
    ...
    if (window == *expected_stalled_window && (!first_skip_ts.has_value() || record.ts < *first_skip_ts)) {
  ...
  for (td::uint32 window = *last_finalized_window + 1; window < *expected_stalled_window; ++window) {
    if (skip_windows.contains(window)) {
      return td::Status::Error(PSTRING() << "scenario unexpectedly produced SkipVote in intermediate window " << window
                                         << " before the intended stalled window " << *expected_stalled_window);
    }
  }

  if (!first_skip_ts.has_value()) {
    return td::Status::Error(
        PSTRING() << "validator #0.0 emitted no SkipVote in stalled window " << *expected_stalled_window
                  << " even though finalization stayed frozen after window " << *last_finalized_window
                  << "; observed skip windows=" << windows_to_string(skip_windows));
  }
  ...
  double spec_lower_bound_s =
      static_cast<double>(FIRST_BLOCK_TIMEOUT.count()) / 1000.0 *
...
td::Status verify_skip_timeout_cap_plateaus_under_stall(const TraceSnapshot& snapshot) {
  ...
  double expected_capped_delay_s =
      static_cast<double>(FIRST_BLOCK_MAX_TIMEOUT.count()) / 1000.0 + TARGET_RATE_MS / 1000.0;
  ...
  if (capped_gap_s > 0.12) {
    return td::Status::Error(PSTRING() << "skip timeout did not plateau at the configured cap: last delays="
                                       << observed_delays[slot_count - 2] << "s and " << observed_delays[slot_count - 1]
                                       << "s");
  }
  if (std::abs(observed_delays[slot_count - 1] - expected_capped_delay_s) > 0.12) {
    return td::Status::Error(PSTRING() << "capped skip delay " << observed_delays[slot_count - 1]
                                       << "s did not match configured cap-based expectation " << expected_capped_delay_s
                                       << "s");
  }
  ...
  for (size_t i = 0; i < skip_count; ++i) {
    ...
    for (const auto& record : snapshot.candidate_deliveries) {
      if (record.src_node_idx == node_idx && record.candidate_id.slot == slot && record.ts > stop_ts + 1e-9) {
        return td::Status::Error(PSTRING() << "silent leader #" << node_idx << " still delivered candidate "
                                           << record.candidate_id << " after stop");
      }
    }
  }
...
td::Status verify_count_threshold_mismatch_preserves_safety(const TraceSnapshot& snapshot) {
  ...
  if (holder_weight >= configured_quorum_weight()) {
    return td::Status::Error(PSTRING() << "attacked slot " << *attacked_slot << " unexpectedly reached quorum weight "
                                       << holder_weight);
  }
...
td::Status verify_offline_cohort_network_handoff_still_finalizes(const TraceSnapshot& snapshot) {
  // Covers a weighted online-cohort handoff under pure network isolation:
  // - validator #0 carries slightly more weight and stays online for the whole test
  // - cohort A = {1, 2, 3} is network-disabled first
  // - the remaining online validators must still accept 100 more blocks while A is offline
  // - then A is re-enabled and cohort B = {4, 5, 6} is disabled immediately, with no grace period
  // - after that handoff, consensus must keep accepting later blocks and every rejoined A validator
  //   must actively rejoin voting and later observe a much more distant post-handoff finalized
  //   height; they need that longer runway to replay backlog and restore steady-state work before
  //   their local trace fully catches up with the live tip again
  TRY_STATUS(verify_adversarial_invariants(snapshot));

  std::optional<td::uint32> target_seqno;
  std::optional<td::uint32> recovery_target_seqno;
  {
    std::scoped_lock lock(test_expectations_mutex);
    target_seqno = test_expectations.cohort_handoff_phase1_target_seqno;
    recovery_target_seqno = test_expectations.cohort_handoff_recovery_target_seqno;
  }
  if (!target_seqno.has_value()) {
    return td::Status::Error("scenario did not register the phase-1 cohort-handoff target seqno");
  }
  if (!recovery_target_seqno.has_value()) {
    return td::Status::Error("scenario did not register the post-handoff recovery target seqno");
  }
  ...
  for (const auto& record : snapshot.network_toggles) {
    for (size_t node_idx : cohort_handoff_first_offline_nodes()) {
    ...
    for (size_t node_idx : cohort_handoff_second_offline_nodes()) {
      if (record.node_idx != node_idx || record.instance_idx != 0) {
        continue;
      }
  ...
  for (size_t node_idx : cohort_handoff_first_offline_nodes()) {
    ...
    if (disable_it == disable_ts_by_first_cohort.end()) {
      return td::Status::Error(PSTRING() << "scenario did not disable first offline cohort node #" << node_idx);
    }
    if (reenable_it == reenable_ts_by_first_cohort.end()) {
      return td::Status::Error(PSTRING() << "scenario did not re-enable first offline cohort node #" << node_idx);
    }
    if (reenable_it->second <= disable_it->second) {
      return td::Status::Error(PSTRING() << "first offline cohort node #" << node_idx
  ...
  if (!std::isfinite(phase1_end_ts) || phase1_end_ts <= phase1_start_ts) {
    return td::Status::Error("scenario did not create a valid interval where the whole first cohort was offline");
  }
  ...
  for (size_t node_idx : cohort_handoff_second_offline_nodes()) {
    ...
    if (disable_it == disable_ts_by_second_cohort.end()) {
      return td::Status::Error(PSTRING() << "scenario did not disable second offline cohort node #" << node_idx);
    }
  ...
  if (earliest_second_disable_ts + 1e-9 < latest_reenable_ts) {
    return td::Status::Error("scenario started disabling the second cohort before the first cohort fully rejoined");
  }
  if (phase2_end_ts <= phase2_start_ts + 1e-9) {
    return td::Status::Error("scenario did not leave any pure handoff interval before restoring cohort B");
  }
  ...
  if (!saw_phase1_progress) {
    return td::Status::Error(PSTRING() << "always-online validator #0.0 never accepted block seqno >= " << *target_seqno
                                       << " while cohort A was offline");
  }

  bool saw_phase2_progress = false;
  td::uint32 highest_phase2_seqno = 0;
  for (const auto& record : snapshot.accepted_blocks) {
    if (record.node_idx == 0 && record.instance_idx == 0 && record.ts > phase2_start_ts + 1e-9 &&
        record.ts + 1e-9 < phase2_end_ts) {
      highest_phase2_seqno = std::max(highest_phase2_seqno, record.block_id.seqno());
      if (record.block_id.seqno() >= *recovery_target_seqno) {
        saw_phase2_progress = true;
      }
    }
  }
  if (!saw_phase2_progress) {
    return td::Status::Error(
        PSTRING() << "always-online validator #0.0 never accepted block seqno >= " << *recovery_target_seqno
                  << " while cohort B was disabled; highest phase-2 seqno=" << highest_phase2_seqno);
  }

  for (size_t node_idx : cohort_handoff_first_offline_nodes()) {
    bool saw_post_handoff_vote = false;
    for (const auto& record : snapshot.protocol_votes) {
      if (record.src_node_idx == node_idx && record.src_instance_idx == 0 && record.ts > phase2_start_ts + 1e-9 &&
          record.ts + 1e-9 < phase2_end_ts && is_notar_or_final_vote(record.vote)) {
        saw_post_handoff_vote = true;
        break;
      }
    }
    if (!saw_post_handoff_vote) {
      return td::Status::Error(PSTRING() << "rejoined cohort-A validator #" << node_idx
                                         << ".0 emitted no Notar/Final vote after the handoff");
    }

    bool saw_post_handoff_finalization = false;
    for (const auto& record : snapshot.finalizations_observed) {
      if (record.node_idx == node_idx && record.instance_idx == 0 && record.ts > phase2_start_ts + 1e-9 &&
          record.id.slot >= *recovery_target_seqno) {
        saw_post_handoff_finalization = true;
        break;
      }
    }
    if (!saw_post_handoff_finalization) {
      return td::Status::Error(PSTRING() << "rejoined cohort-A validator #" << node_idx
                                         << ".0 never observed post-handoff finalization at seqno "
...
td::Status verify_offline_cohort_restart_handoff_still_finalizes(const TraceSnapshot& snapshot) {
  // Covers the same weighted handoff, but with real process restarts instead of mere network loss:
  // - validator #0 stays online throughout and carries slightly more weight than the others
  // - cohort A = {1, 2, 3} is fully stopped first, while the remaining validators must still
  //   accept 100 more blocks
  // - then A is started again and cohort B = {4, 5, 6} is stopped immediately, without waiting for
  //   the restarted nodes to pre-sync first
  // - after that hard handoff, consensus must keep finalizing and every restarted A validator must
  //   both emit Notar/Final traffic and later observe a much more distant post-handoff finalized
  //   height; restarting validators need that extra time to rebuild their working frontier before
  //   their local trace fully catches up with the live tip again
  TRY_STATUS(verify_adversarial_invariants(snapshot));
  ...
  if (!target_seqno.has_value()) {
    return td::Status::Error("scenario did not register the phase-1 restart-handoff target seqno");
  }
  if (!recovery_target_seqno.has_value()) {
    return td::Status::Error("scenario did not register the post-handoff restart recovery target seqno");
  }
  ...
  for (size_t node_idx : cohort_handoff_first_offline_nodes()) {
    ...
    if (stop_it == stop_ts_by_first_cohort.end()) {
      return td::Status::Error(PSTRING() << "scenario did not stop first restart cohort node #" << node_idx);
    }
    if (start_it == start_ts_by_first_cohort.end()) {
      return td::Status::Error(PSTRING() << "scenario did not restart first restart cohort node #" << node_idx);
    }
    if (start_it->second <= stop_it->second) {
      return td::Status::Error(PSTRING() << "restart cohort node #" << node_idx << " has invalid stop/start ordering");
    }
  ...
  if (!std::isfinite(phase1_end_ts) || phase1_end_ts <= phase1_start_ts) {
    return td::Status::Error("scenario did not create a valid interval where the whole first cohort was stopped");
  }
  ...
  for (size_t node_idx : cohort_handoff_second_offline_nodes()) {
    ...
    if (stop_it == stop_ts_by_second_cohort.end()) {
      return td::Status::Error(PSTRING() << "scenario did not stop second restart cohort node #" << node_idx);
    }
  ...
  if (earliest_second_stop_ts + 1e-9 < latest_restart_ts) {
    return td::Status::Error("scenario stopped the second cohort before the first cohort fully restarted");
  }
  if (phase2_end_ts <= phase2_start_ts + 1e-9) {
    return td::Status::Error("scenario did not leave any pure restart handoff interval before restoring cohort B");
  }
  ...
  if (!saw_phase2_progress) {
    return td::Status::Error(
        PSTRING() << "always-online validator #0.0 never accepted block seqno >= " << *recovery_target_seqno
                  << " while cohort B was stopped; highest phase-2 seqno=" << highest_phase2_seqno);
  }
  ...
  for (size_t node_idx : cohort_handoff_first_offline_nodes()) {
    ...
    if (!saw_post_handoff_vote) {
      return td::Status::Error(PSTRING() << "restarted cohort-A validator #" << node_idx
                                         << ".0 emitted no Notar/Final vote after the handoff");
    }
  ...
  for (size_t node_idx : cohort_handoff_second_offline_nodes()) {
    ...
    for (const auto& record : snapshot.protocol_votes) {
      if (record.src_node_idx == node_idx && record.src_instance_idx == 0 && record.ts > stop_ts + 1e-9 &&
        ...
        return td::Status::Error(PSTRING()
                                 << "stopped cohort-B validator #" << node_idx << ".0 still emitted a vote after stop");
      }
    }
    for (const auto& record : snapshot.protocol_certificates) {
      if (record.src_node_idx == node_idx && record.src_instance_idx == 0 && record.ts > stop_ts + 1e-9 &&
          record.ts + 1e-9 < restart_ts) {
        return td::Status::Error(PSTRING() << "stopped cohort-B validator #" << node_idx
...
td::Status verify_standstill_with_byzantine_withholding_still_recovers(const TraceSnapshot& snapshot) {
  ...
  double standstill_rebroadcast_deadline =
      *honest_disable_ts + static_cast<double>(STANDSTILL_TIMEOUT.count()) / 1000.0 * 0.75;
  ...
  if (!saw_rebroadcast) {
    return td::Status::Error(
        "honest validators did not rebroadcast any previously seen votes/certificates during the standstill");
  }
...
td::Status verify_no_notar_observed_skip_wins(const TraceSnapshot& snapshot) {
  ...
  for (const auto& record : snapshot.protocol_certificates) {
    if (auto* notar = std::get_if<simplex::NotarizeVote>(&record.vote.vote);
        notar != nullptr && notar->id.slot == *attacked_slot) {
      return td::Status::Error(PSTRING() << "slot " << *attacked_slot << " still reached a notarization certificate");
    }
  }
...
td::Status verify_later_slot_waits_for_earlier_notar_proof(const TraceSnapshot& snapshot) {
  ...
  if (!parent_notar_observed_ts.has_value()) {
    return td::Status::Error(PSTRING() << "validator #0.0 never observed notarization for parent slot "
                                       << *parent_slot);
  }
...
td::Status verify_replay_stale_certificate_does_not_disturb_progress(const TraceSnapshot& snapshot) {
  ...
  for (const auto& record : snapshot.protocol_votes) {
    ...
    if (slot == *stale_slot) {
      return td::Status::Error(PSTRING() << "honest validator #" << record.src_node_idx
                                         << " resumed voting on stale slot " << *stale_slot << " after replay");
    }
  }
  ...
  for (const auto& record : snapshot.protocol_certificates) {
    ...
    auto slot = vote_trace_slot(record.vote);
    if (slot == *stale_slot) {
      return td::Status::Error(PSTRING() << "honest validator #" << record.src_node_idx << " rebroadcast stale slot "
                                         << *stale_slot << " after Byzantine replay");
    }
  }
...
td::Status verify_notarized_and_skipped_slot_coexist_without_finalization(const TraceSnapshot& snapshot) {
  ...
  if (!saw_skip_cert) {
    return td::Status::Error(PSTRING() << "forked slot " << *forked_slot << " never reached a skip certificate");
  }
...
td::Status verify_equivocating_leader_does_not_create_two_notars_or_finals(const TraceSnapshot& snapshot) {
  ...
  if (!notarized_by_instance.contains(0) || !notarized_by_instance.contains(1) || notarized_by_instance[0].empty() ||
      notarized_by_instance[1].empty()) {
...
td::Status verify_later_slot_waits_for_gap_skip_proof(const TraceSnapshot& snapshot) {
  ...
  for (const auto& record : snapshot.protocol_certificates) {
    if (auto* skip = std::get_if<simplex::SkipVote>(&record.vote.vote);
        skip != nullptr && skip->slot == *gap_slot && (!record.dst_node_idx.has_value() || *record.dst_node_idx == 0)) {
...
td::Status verify_multi_slot_equivocation_window_preserves_safety(const TraceSnapshot& snapshot) {
  ...
  if (!saw_later_honest_finalization) {
    return td::Status::Error(
        PSTRING() << "honest validators never finalized past multi-slot equivocation window ending at slot "
                  << window_end_slot);
  }
...
td::Status verify_invalid_full_candidate_does_not_notarize(const TraceSnapshot& snapshot) {
  ...
  if (!saw_skip_cert) {
    return td::Status::Error(PSTRING() << "invalid full-candidate slot " << *attacked_slot << " never cleared by Skip");
  }
  return td::Status::OK();
}
...
td::Status verify_restart_after_eight_non_mc_finalized_blocks_reaccepts_pending_candidates(
  ...
  if (!saw_node0_catchup_finalization) {
    return td::Status::Error(
        "restarted validator #0.0 never observed finalization after the empty-slot catch-up point");
  }
...
td::Status verify_good_candidate_with_wrong_slot_parent_hash_does_not_resolve(const TraceSnapshot& snapshot) {
  ...
  for (const auto& record : snapshot.protocol_votes) {
    if (auto* notar = std::get_if<simplex::NotarizeVote>(&record.vote.vote);
      ...
      return td::Status::Error(PSTRING() << "honest validator #" << record.src_node_idx
                                         << " voted Notar for wrong-parent-hash candidate " << *attacked_candidate_id);
    }
  }
  for (const auto& record : snapshot.protocol_certificates) {
    if (auto* notar = std::get_if<simplex::NotarizeVote>(&record.vote.vote);
  ...
  if (!saw_skip_cert) {
    return td::Status::Error(PSTRING() << "attacked slot " << attacked_candidate_id->slot << " never cleared by Skip");
  }
...
std::optional<std::pair<td::uint32, TestMessageFilters::ProtocolKind>> classify_protocol_message(
    const TestSimplexBus& bus, size_t src_node_idx, size_t src_instance_idx, size_t dst_node_idx, td::Slice data) {
 ...
 public:
  ...
  void handle(BusHandle bus, std::shared_ptr<const OutgoingProtocolMessage> message) {
    ...
    auto trace_message = [&](size_t dst_idx) {
      ...
      if (auto vote = try_decode_protocol_vote(test_bus, bus->local_id.idx.value(), instance_idx_, dst_idx,
    ...
    if (message->recipient.has_value()) {
      ...
      if (should_drop_protocol_message(test_bus, bus->local_id.idx.value(), instance_idx_, message->recipient->value(),
                                       message->message.data.as_slice())) {
        return;
      }
  ...
  td::actor::Task<ProtocolMessage> process(BusHandle bus, std::shared_ptr<OutgoingOverlayRequest> message) {
    auto& test_bus = dynamic_cast<const TestSimplexBus&>(*bus);
    size_t dst_node_idx = rewrite_candidate_request_destination(
        bus->local_id.idx.value(), instance_idx_, message->destination.value(), message->request.data.as_slice());
    if (!test_bus.trace_sink.empty()) {
      auto record = decode_overlay_request(bus->local_id.idx.value(), instance_idx_, dst_node_idx, message->timeout,
  ...
  void handle(BusHandle bus, std::shared_ptr<const CandidateGenerated> event) {
    ...
    if (!test_bus.trace_sink.empty()) {
      td::actor::send_closure(test_bus.trace_sink, &TestTraceSink::record_candidate_generated,
                              bus->local_id.idx.value(), instance_idx_, event->candidate->id,
                              event->candidate->parent_id, event->candidate->leader, event->candidate->is_empty(),
                              event->candidate->block_id());
    }
  }
 ...
 public:
  ...
  td::actor::Task<> on_block_accepted(size_t node_idx, size_t instance_idx, td::Ref<BlockData> block,
    ...
    if (last_accepted_block_.seqno() < seqno && signatures->is_final()) {
      ...
      {
        std::scoped_lock lock(test_expectations_mutex);
        freeze_synthetic_mc_finalization = test_expectations.synthetic_mc_finalization_freeze_seqno.has_value() &&
  ...
  td::actor::Task<> set_selective_protocol_drop_for_test(size_t src_node_idx, size_t src_instance_idx,
                                                         std::vector<size_t> dst_node_indices, td::uint32 start_slot,
                                                         td::uint32 end_slot, TestMessageFilters::ProtocolKind kind) {
    std::scoped_lock lock(test_message_filters_mutex);
    test_message_filters.protocol_drop_rules.push_back(TestMessageFilters::ProtocolDropRule{
        .src_node_idx = src_node_idx,
        .src_instance_idx = src_instance_idx,
  ...
  td::actor::Task<> set_malformed_candidate_responses_for_test(
      size_t src_node_idx, size_t src_instance_idx, size_t dst_node_idx, size_t budget,
  ...
  td::actor::Task<> set_candidate_starvation_expectations_for_test(std::vector<size_t> node_indices, td::uint32 slot,
                                                                   std::vector<size_t> dropped_nodes) {
    std::scoped_lock lock(test_expectations_mutex);
    test_expectations.adversarial_nodes = std::move(node_indices);
  ...
  td::actor::Task<> inject_vote_for_test(size_t src_node_idx, std::vector<size_t> dst_node_indices,
                                         simplex::Vote vote) {
    ...
    co_return co_await inject_protocol_message_for_test(src_node_idx, std::move(dst_node_indices),
  ...
  BlockCandidate make_synthetic_block_candidate_for_test(td::uint32 slot, BlockSeqno prev_seqno, td::uint64 tag,
    ...
    info.prev_ref = make_ext_blk_ref(
        BlockIdExt(BlockId(SHARD, prev_seqno), gen_shard_state(prev_seqno)->get_hash().bits(), td::Bits256{}),
    ...
    rand_data.as_slice()
        .truncate(sizeof(tag))
        .copy_from(td::Slice{reinterpret_cast<const td::uint8*>(&tag), sizeof(tag)});
    td::Ref<vm::Cell> block_extra = vm::CellBuilder{}.store_bytes(rand_data.as_slice()).finalize_novm();
    td::Ref<vm::Cell> block_root = vm::CellBuilder{}
                                       .store_long(0x11ef55aa, 32)
                                       .store_long(-111, 32)
 ...
 private:
  void apply_test_case_defaults() {
    ...
    if (TEST_CASE == TestCase::LongStallClearsSkippedSlotsThenHonestLeaderFinalizes) {
      ...
      STANDSTILL_TIMEOUT = std::chrono::milliseconds{500};
    }
    if (TEST_CASE == TestCase::LongStallClearsSkippedSlotsThenHonestLeaderFinalizes) {
      if (N_NODES == 8) {
        N_NODES = 4;
    ...
    if (TEST_CASE == TestCase::OfflineCohortNetworkHandoffStillFinalizes ||
    ...
    if (TEST_CASE == TestCase::StandstillWithByzantineWithholdingStillRecovers) {
      ...
      STANDSTILL_TIMEOUT = std::chrono::milliseconds{1000};
    }
    ...
    if (TEST_CASE == TestCase::SkipTimeoutCapPlateausUnderStall) {
      ...
      FIRST_BLOCK_TIMEOUT = std::chrono::milliseconds{50};
  ...
  td::actor::Task<> wait_for_finalization_past_slot_on(size_t node_idx, size_t instance_idx, td::uint32 slot,
    ...
    co_return td::Status::Error(PSTRING() << "timeout waiting for finalization past slot " << slot << " on validator #"
  ...
  td::actor::Task<> run_test_case_scenario() {
    ...
    if (TEST_CASE == TestCase::InvalidCandidateSignatureDoesNotResolve) {
      ...
      if (!target_id.has_value()) {
        co_return td::Status::Error(
            "could not find a candidate notarized by the honest serving peer for the invalid-signature Rule 2 "
            "scenario");
      }
      co_await set_candidate_resolution_target_for_test(*target_id);
    ...
    if (TEST_CASE == TestCase::SeveralWindowsFullySkippedThenProgressResumes) {
      ...
      co_await wait_for_finalization_past_slot_on(1, 0, first_silent_slot - static_cast<td::uint32>(N_NODES) + 2, 5.0);
      co_await clear_traces();
      for (size_t node_idx : silent_nodes) {
        co_await stop_instance(node_idx, 0);
      }
    ...
    if (TEST_CASE == TestCase::GoodCandidateWithWrongSlotParentHashDoesNotResolve) {
      ...
      auto candidate = make_synthetic_full_candidate_for_test(attacked_slot, wrong_parent, PeerValidatorId{1},
                                                              attacked_slot - 1, 0xC0FFEE00ULL + attacked_slot);
      {
        std::scoped_lock lock(test_expectations_mutex);
    ...
    if (TEST_CASE == TestCase::OfflineCohortNetworkHandoffStillFinalizes) {
      // This is the stronger rotating-online-cohort liveness test under pure network isolation:
      // - 9 validators participate, and validator #0 carries slightly more weight than the others
      // - cohort A = {1, 2, 3} is disconnected first
      // - with validator #0 still online, the remaining online validators must keep going long
      //   enough to accept 100 more blocks despite the permanently recurring skipped leader slots
      // - then cohort A is re-enabled and cohort B = {4, 5, 6} is disconnected immediately,
      //   without any grace period in between
      // - after that hard handoff, we require continued progress on validator #0 immediately and
      //   then keep cohort B offline until validator #0 reaches a much later block; the rejoined
      //   validators need that longer runway to replay backlog, restore steady-state work, and
      //   observe finalized height near the live tip again
      // - once that observation window is over, cohort B is re-enabled purely as cleanup so the
      //   harness can shut the whole test network down without leaving peers stuck on dead links
      co_await wait_for_finalization_on(0, 0, 5.0);
      auto snapshot = co_await td::actor::ask(trace_sink_, &TestTraceSink::snapshot);
      td::uint32 highest_accepted_seqno = FIRST_PARENT.seqno();
      for (const auto& record : snapshot.accepted_blocks) {
      ...
      td::uint32 target_seqno = highest_accepted_seqno + COHORT_HANDOFF_BLOCK_COUNT;
      td::uint32 recovery_target_seqno = target_seqno + COHORT_HANDOFF_RECOVERY_BLOCK_DELTA;
      {
        std::scoped_lock lock(test_expectations_mutex);
        test_expectations.cohort_handoff_phase1_target_seqno = target_seqno;
        test_expectations.cohort_handoff_recovery_target_seqno = recovery_target_seqno;
      }
      ...
      co_await wait_for_accepted_block_seqno_on(0, 0, recovery_target_seqno, 25.0);
      co_await td::actor::coro_sleep(td::Timestamp::in(DURATION));
      for (size_t node_idx : cohort_handoff_second_offline_nodes()) {
        co_await set_instance_network_disabled_for_test(node_idx, 0, false);
      }
    ...
    if (TEST_CASE == TestCase::OfflineCohortRestartHandoffStillFinalizes) {
      // This is the same rotating-online-cohort handoff, but with real restarts instead of just
      // cutting the network:
      // - validator #0 again stays online and carries slightly more weight
      // - cohort A = {1, 2, 3} is fully stopped first, but the rest of the network must still
      //   accept 100 more blocks
      // - then cohort A is started again and cohort B = {4, 5, 6} is stopped immediately, with
      //   no waiting for the restarted nodes to pre-sync before the old cohort disappears
      // - the expected outcome is that consensus keeps finalizing later blocks after the swap, and
      //   we keep cohort B stopped until validator #0 reaches a much later block because the
      //   restarted validators need that extra runway to replay backlog, restore steady-state work,
      //   and observe finalized height near the live tip again
      // - once that observation window is complete, cohort B is started again purely for cleanup
      //   so suite shutdown does not wait on peers that are still timing out old overlay work
      co_await wait_for_finalization_on(0, 0, 5.0);
      auto snapshot = co_await td::actor::ask(trace_sink_, &TestTraceSink::snapshot);
      td::uint32 highest_accepted_seqno = FIRST_PARENT.seqno();
      for (const auto& record : snapshot.accepted_blocks) {
    ...
    if (TEST_CASE == TestCase::EquivocatingLeaderDoesNotCreateTwoNotarsOrFinals) {
      ...
      while (!deadline.is_in_past()) {
        ...
        if (candidates_by_instance.size() == 2 && candidates_by_instance.at(0) != candidates_by_instance.at(1)) {
          co_await set_equivocation_expectations_for_test(attacked_slot,
                                                          {candidates_by_instance.at(0), candidates_by_instance.at(1)});
          co_await td::actor::coro_sleep(td::Timestamp::in(0.4));
      ...
      co_return td::Status::Error(PSTRING()
                                  << "timeout waiting for two distinct same-key candidates on slot " << attacked_slot);
    }
    if (TEST_CASE == TestCase::MultiSlotEquivocationWindowPreservesSafety) {
      // Covers B2 and D1 from the adversarial plan:
    ...
    if (TEST_CASE == TestCase::CandidateResolutionRecovery) {
      ...
      if (!target_id.has_value()) {
        co_return td::Status::Error(
            "could not find a candidate notarized by the serving peer for Rule 2 recovery test");
      }
      co_await set_candidate_resolution_target_for_test(*target_id);
  ...
  td::actor::Task<> run_inner() {
    ...
    if (!NODE_WEIGHTS_OVERRIDE.empty() && NODE_WEIGHTS_OVERRIDE.size() != N_NODES) {
      co_return td::Status::Error(PSTRING() << "configured " << NODE_WEIGHTS_OVERRIDE.size() << " node weights for "
                                            << N_NODES << " validators");
    }
  ...
  void start_instance(size_t node_idx, size_t instance_idx) {
    ...
    bus->config = NewConsensusConfig{.max_block_size = 1 << 20,
                                     .max_collated_data_size = 1 << 20,
                                     .slots_per_leader_window = SLOTS_PER_LEADER_WINDOW,
                                     .noncritical_params = {
                                         .target_rate = std::chrono::milliseconds{TARGET_RATE_MS},
                                         .first_block_timeout = FIRST_BLOCK_TIMEOUT,
                                         .first_block_timeout_multiplier = FIRST_BLOCK_TIMEOUT_MULTIPLIER,
                                         .first_block_timeout_cap = FIRST_BLOCK_MAX_TIMEOUT,
