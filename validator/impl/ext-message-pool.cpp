...
void ExtMessagePool::install_collator_queue(ShardIdFull shard, std::unique_ptr<ExtMsgCallback> callback) {
  // Compute shard key range for splitting
  ...
  for (auto it = ext_msgs_.rbegin(); it != ext_msgs_.rend(); ++it) {
    Treap in_shard;
    if (hi_prefix_plus1 == 0) {
      // Full range — shard covers lo_prefix to max. Just split once, take the right half.
      auto [_, right] = it->second.ext_messages_.split(shard_lo);
      in_shard = std::move(right);
    } else {
      MessageId shard_end{AccountIdPrefixFull{shard.workchain, hi_prefix_plus1}, Bits256::zero()};
      auto [_, mid, __] = it->second.ext_messages_.split_range(shard_lo, shard_end);
      in_shard = std::move(mid);
    }
    if (!in_shard.empty()) {
