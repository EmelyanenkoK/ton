...
class PoolImpl : public td::actor::SpawnsWith<Bus>, public td::actor::ConnectsTo<Bus> {
 ...
 public:
  ...
  void start_up() override {
    ...
    for (const auto &cert : bus.bootstrap_certificates) {
      ...
      cert->copy_and_downcast([&](auto typed_cert) { handle_saved_certificate(*slot, std::move(typed_cert)); });
    }
    ...
    for (const auto &vote : bus.bootstrap_votes) {
      ...
      handle_our_vote(vote, /*tolerate_conflicts=*/true).start().detach();
    }
 ...
 private:
  ...
  td::actor::Task<> handle_our_vote(Vote vote, bool tolerate_conflicts = false) {
    auto &bus = *owning_bus();
  ...
  td::actor::Task<> handle_certificate(CertificateRef<Vote> cert) {
    ...
    std::move(cert.unique_write()).consume_and_downcast([&](auto cert) {
